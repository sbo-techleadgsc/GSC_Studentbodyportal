...
4. **Redeploy your project** (required for environment variable changes to take effect)
...
- Vercel: Delete the environment variable or set it to `false` (requires redeploy)
...
- **Redeployment is required** when changing the environment variable on Vercel
- The static HTML version at `/under-maintenance.html` is available as a fallback

## Alternative: Direct Access to Maintenance Page

You can also access the maintenance page directly at `/under-maintenance.html` without enabling maintenance mode. This is useful for:
- Testing the maintenance page design
- Showing the page during planned maintenance windows
- Providing a link to users during extended outages
